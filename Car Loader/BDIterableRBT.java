...
public class BDIterableRBT<T extends Comparable<T>> implements IterableRBTADT<T> {
    ...
    public void forEach(Consumer<? super T> action) {
        IterableRBTADT.super.forEach(action);
    }

    @Override
    public Spliterator<T> spliterator() {
        return IterableRBTADT.super.spliterator();
    }
    ...
    protected void enforceRBTreePropertiesAfterInsert(Node<T> redNode) {
        // Checks if the parent is red, which violate red-black tree properties
        if (redNode.parent == null || redNode.parent.blackHeight == 1) {
            return;
        }

        // Assigning variables
        Node<T> par = redNode.parent;
        Node<T> gpar = par.parent;
        Node<T> aunt;

        // Assigns aunt based on whether parent is left child or right child
        if (par.isLeftChild()) {
            aunt = par.parent.rightChild;
        } else {
            aunt = par.parent.leftChild;
        }

        // Checks cases: 1. if aunt is red, 2. aunt is black
        if (aunt != null && aunt.blackHeight == 0) {
            // Performs a recolor operation
            par.blackHeight = 1;
            aunt.blackHeight = 1;
            gpar.blackHeight = 0;

            // Recursively checks the red black tree properties
            enforceRBTreePropertiesAfterInsert(gpar);
        } else {
            // Performs parent-child rotation if child is opposite child of grandparent.
            if ((redNode.isLeftChild() && (aunt == gpar.leftChild)) || (!redNode.isLeftChild() && aunt != gpar.leftChild)) {
                rotate(redNode, par);
                par = redNode;
            }

            // Performs parent-grandparent rotation and color swaps
            rotate(par, gpar);
            par.blackHeight = 1;
            gpar.blackHeight = 0;
        }
    ...
    private void rotate(Node<T> child, Node<T> parent) throws IllegalArgumentException {
        ...
        if (parent.leftChild == child) {
            Node<T> c = child.rightChild;
            Node<T> aunt = parent.rightChild;

            parent.leftChild = c;
            child.rightChild = parent;
        } else if (parent.rightChild == child) {
            Node<T> c = child.leftChild;
            Node<T> aunt = child.rightChild;
