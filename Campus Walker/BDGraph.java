...
public class BDGraph<NodeType,EdgeType extends Number> implements GraphInterface<NodeType,EdgeType> {
    ...
    public BDGraph() { vertices = new Hashtable<>(); }
    ...
    protected class Path implements Comparable<Path> {
        ...
        public Path(Path copyPath, Edge extendBy) {
            ...
            // deep copy of the dataSequence of copyPath
            this.dataSequence = new LinkedList<>();
            for (int i = 0; i < copyPath.dataSequence.size(); i++) {
                Vertex vertexToAdd = new Vertex(copyPath.dataSequence.get(i));
                this.dataSequence.add(vertexToAdd.data);
            }
    ...
    protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
        if (start == null || end == null) {
            throw new NoSuchElementException("Start or end nodes can not be null");
        }
        ...
        PriorityQueue<Path> pathsQueue = new PriorityQueue<Path>();
        List<Vertex> visitedNodes = new LinkedList<>(); // LinkedList to keep track of visited vertices
        Hashtable<Vertex, Path> pathsToEachVertex = new Hashtable<>(); // Hashtable to keep track of each path from starting vertex to all other vertices
        boolean pathToVertexExists = false;
        Path currPath;

        pathsQueue.add(new Path(startVertex));

        while(!pathsQueue.isEmpty()) {
            currPath = pathsQueue.poll();
            // check if end vertex of current path has already been visited
            for (Vertex v : visitedNodes) {
                if (v.data.equals(currPath.end.data)) {
                    pathToVertexExists = true;
                }
            }

            if (pathToVertexExists == false) {
                visitedNodes.add(currPath.end);
                // since first time visiting vertex, the path to vertex is shortest path
                pathsToEachVertex.put(currPath.end, currPath);
                // construct new paths for each outgoing edge of the visited vertex, add each path to priority queue
                for (Edge e : currPath.end.edgesLeaving) {
                    pathsQueue.add(new Path(currPath, e));
                }
            }
            pathToVertexExists = false;
        }
        Path shortestPath = pathsToEachVertex.get(endVertex);
    ...
    public List<List<NodeType>> getAllPaths(NodeType start) {
        List<List<NodeType>> returnList = new ArrayList<>();
        for (Map.Entry<NodeType,Vertex> entry : vertices.entrySet()) {
            try {
                returnList.add(shortestPath(start, entry.getKey()));
            } catch (Exception e) {
                continue;
            }
        }
